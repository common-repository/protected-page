...
class Wewp_Guard_Content
{
    ...
    public function checkDisplayForm()
    {
        ...
        if ($this->isCPTLimitAuthorization()) {
            $content_levels = get_option('protect_post_type_by_user_roles');
            if (!isset($content_levels[$post->post_type]) || empty($content_levels[$post->post_type]['roles'])) {
                wp_redirect('/');
                exit;
            }

            if (!$this->validateCPTAuthorization($content_levels[$post->post_type]['roles'])) {
                global $post;

                $options = get_option('protect_post_type_by_user_roles');
                $post_id_for_redirect = $options[$post->post_type]['redirect_when_user_has_no_access'];
                wp_redirect(get_page_link($post_id_for_redirect));
        ...
        if (isset($post_protect_by_user_role) && !empty($post_protect_by_user_role)) {
            global $post;

            if (!$this->validateCPTAuthorization($post_protect_by_user_role)) {
                $redirect_post_id = get_post_meta($page_id, 'protect_page_redirect_user_has_no_access', true);
                wp_redirect(get_page_link($redirect_post_id));
                exit;
            }
            return false;
        }
    ...
    private function validateCPTAuthorization($content_levels)
    {
        global $post;
        ...
        foreach ($logged_user_roles as $key => $logged_user_role) {
            $hasAuthorization = in_array($logged_user_role, $content_levels);
